  ...
  <div class="xxx">
    <header class="home_menu">
      ...
      <nav>
        <ul class="nav_links">
          <li class="nav_li">
            <a href="http://localhost:63342/test1/project/index.html">主页</a>
          </li>
          <li class="nav_li">
            <a href="http://localhost:5173/">标题1</a>
          </li>
          <li class="nav_li">
            <a href="http://localhost:63342/test1/project/test2.html">标题2</a>
          </li>
          <li class="nav_li">
            <a href="http://localhost:63342/test1/project/和弦图.html">标题3</a>
          </li>
        </ul>
      </nav>
    </header>
    ...
    <div @click="handleOutsideClick">
      <div ref="mapContainer" class="box_map">
        <div ref="map" style="width: 700px; height: 550px; border: solid red 1px" />
<!--        <span @click="toback" class="span_map">宏观</span>-->
<!--        <span @click="toback" class="span_map" style="left:100px;">微观</span>-->
<!--        <span :style="{fontSize: macroFontSize + 'px'}" class="span_map">宏观</span>-->
<!--        <span :style="{fontSize: microFontSize + 'px'}" class="span_map" style="left:100px;">微观</span>-->
        <span :style="{fontSize: macroFontSize + 'px'}" @click="toback" class="span_map">宏观</span>
        <span :style="{fontSize: microFontSize + 'px'}" @click="toback" class="span_map" style="left:100px;">微观</span>
        <span class="地图说明">©2022 高德软件 GS京(2022)1061号，数据来源于高德开放平台</span>
      </div>
    </div>


    <div id="container_text">
      <div class="header_text_和弦">
        <span class="标题栏">数锦寰宇</span>
        <span class="内容栏">
...
import hainan from "../json/chinamap/460000.json"
...
export default {
  ...
  methods: {
    ...
    initParamsCity(name) {
      ...
      // 有返回记录
      // 要有城市映射
      const map = {中国: chinaJson, 广西壮族自治区: guangxi, 黑龙江省: heilongjiang, 江苏省: jiangsu, 浙江省: zhejiang,
        安徽省: anhui, 江西省: jiangxi, 山东省: shandong, 河南省: henan, 福建省: fujian, 广东省: guangdong, 贵州省: guizhou,
        海南省: hainan, 湖南省: hunan, 云南省: yunnan, 湖北省: hubei, 重庆市: chongqing, 陕西省: shanxi, 辽宁省: liaoning,
        吉林省: jilin, 内蒙古自治区: neimenggu, 新疆维吾尔自治区: xinjiang, 西藏自治区: xizang, 四川省: sichuan, 青海省: qinghai,
        甘肃省: gansu, 宁夏回族自治区: ningxia, 山西省: shanxisheng, 河北省: hebei, 上海市: shanghai, 北京市: beijing,
        天津市: tianjin, 台湾省: taiwan, 香港特别行政区: xianggang, 澳门特别行政区: aomen,
      };
      const jiangsuData = [
      ...
      const shanghaiData = [
        {name: '徐光启', value: [121.5008, 31.0930]},
      ]
      const zhejiangData = [
...
//陕西
      ...
      // 设置实例大小
      this.myChart = echarts.init(this.$refs.map, null, { width: 700, height: 550 });
          ...
          ...(mapname !== 'china' ? [
            {
              ...
              symbol: 'image://./src/svg/导航倒影.svg', // 指定自定义图标的路径
              zlevel: 10,
            }
          ] : []),
        ],
